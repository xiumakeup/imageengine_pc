...
namespace TestDemo
{
    public partial class EyewarpForm : Skin_Mac
    {
        public EyewarpForm(Bitmap srcBitmap, int[] landmark)
        {
            InitializeComponent();
        ...
        private void eyeWarpButton1_Click_1(object sender, EventArgs e)
        {
            this.DialogResult = DialogResult.OK;
            this.Close();
        }

        private void eyeWarpHScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            mForm = (Form1)this.Owner;
            int warp_ratio = eyeWarpHScrollBar.Value;
            mForm.RefreshDisplay(zMakeup.DoEyeWarp(curBitmap, _landMark, warp_ratio));
            textBox1.Text = eyeWarpHScrollBar.Value.ToString();   
        }
    }
}
