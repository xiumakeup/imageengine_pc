...
namespace TestDemo
{
    public partial class FaceLiftForm : Skin_Mac
    {
        public FaceLiftForm(Bitmap srcBitmap, int[] landmark)
        {
            InitializeComponent();
        ...
        private void FaceLiftButton1_Click(object sender, EventArgs e)
        {
            this.DialogResult = DialogResult.OK;
            this.Close();
        }

        private void FaceLiftHScrollBar_Scroll(object sender, ScrollEventArgs e)
        {
            mForm = (Form1)this.Owner;
            //int lift_ratio = FaceLiftHScrollBar.Value * 14 / 100;
            int lift_ratio = FaceLiftHScrollBar.Value / 2;
            mForm.RefreshDisplay(zMakeup.DoFaceLift(curBitmap, _landMark, lift_ratio));
            textBox1.Text = FaceLiftHScrollBar.Value.ToString();
        }
